  ...
  useEffect(() => {
    if (socket) {
      socket.on("receive-message", (data) => {
        ...
        dipatch(updateGroupListOrder(data?.chatId));
      });
    }
  }, [socket]);
  useEffect(() => {
    const getGroups = async () => {
      try {
        ...
        dipatch(setGroupList(data?.groups));
      } catch (error) {
        console.log(error);
      }
    };
      ...
      <div className="flex flex-col py-2 pt-4">
        {searchedGroup.map(({ group, chatId, lastMessage }) => (
          <div
            ...
            key={group?._id}
            className={`flex items-center gap-3 p-3 mx-2 rounded-lg cursor-pointer transition-all ${
